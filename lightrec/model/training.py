...
class params:
    ...
    """
    def __init__(self, for_model="General", file=None, **kwargs):
        """
        Args:
            for_model (str, optional): the model that need this param. Defaults to "General".
            file (str, optional): A yaml file with configures. Defaults to None.
        """
        self._dict = {}
        self._dict.update(kwargs)
        self._model = for_model
        if file is not None:
            self.open(file)
...
def dict2table(table: dict, headers='row'):
    """turn dict object into organized table
...
def TO(*tensors, **kwargs):
    ...
    Args:
        device (str, optional): 'cuda' or 'cpu', default 'cpu'
    Returns:
        tensor: on device
    """
